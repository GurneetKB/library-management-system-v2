<template>
  <div class="modal" id="BookResource" tabindex="-1" role="dialog">
        ...
        <div class="modal-body">
          ...
          <div class="form-floating">
            <select class="form-select" id="floatingSelect" v-model="newBook.section_id" aria-label="Section" required>
              ...
              <option v-for="section in sections" :key="section.genre_id" :value="section.genre_name">{{section.genre_name}}</option>
            </select>
    ...
    async updateSection() {
      try {
        const response = await axios.post(
          'http://127.0.0.1:5000/api/books/new',
          {
            book_title: this.newBook.title,
            book_view_link: this.newBook.view,
            book_edit_link: this.newBook.edit,
            book_description: this.newBook.description,
            book_creation_date: this.newBook.date,
            book_price: this.newBook.price,
            book_stock: this.newBook.stock,
            book_author: this.newBook.author,
            book_section_id: this.newBook.section_id
          },
          {
            headers: {
